...
String.prototype.toRegExp = function() {
    ...
    if (lastSlash > 1) {
        var restoredRegex = new RegExp(this.slice(1, lastSlash), this.slice(lastSlash + 1));
...
Module.register("MMM-TelegramBot",
{
        ...
        if (handler.args !== null) {
            ...
            this.commands.forEach((c) => {
                if (c.command == target) {
                    ...
                    text += "\n";
                    text += ((c.moduleName) ? (this.translate('TELBOT_HELP_SERVED', {"module":c.moduleName})) : "");
                    text += "\n";
                }
            });
        }
    ...
    TELBOT_alert: function (command, handler) {
        ...
        var alerttime = (this.config.alertTimer * 1000);
                ...
                MM.getModules().enumerate((m) => {
                    if (m.name !== 'MMM-TelegramBot') {
                        if (typeof m.getCommands == 'function') {
                            ...
                            if (Array.isArray(tc)) {
                                tc.forEach((c) => { this.registerCommand(m, c) });
                            }
                        }
                    }
                });
